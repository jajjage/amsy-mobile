...
const isValid = validateRpIdHash(authData.rpIdHash, "amsydatasub.com");
...
const crypto = require('crypto');
const expected = crypto.createHash('sha256').update('amsydatasub.com').digest();
...
async function verifyBiometricRegistration(
  ...
  if (!validateRpIdHash(authData.rpIdHash, "amsydatasub.com")) {
    throw new Error('RP ID hash does not match amsydatasub.com');
  }
  ...
  if (clientDataJSON.origin !== 'https://amsydatasub.com') {
...
async function verifyBiometricRegistration(request) {
  ...
  const crypto = require('crypto');
  const expectedHash = crypto.createHash('sha256').update('amsydatasub.com').digest();
...
const rpIdHash = crypto.createHash('sha256').update('amsydatasub.com').digest('hex');
