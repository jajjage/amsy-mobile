...
| **RP ID Hash** | Zeros | SHA256("amsydatasub.com") |
...
if (build === 'development') {
  ...
  const expected = sha256("amsydatasub.com");
...
const rpIdHash = sha256("amsydatasub.com");
...
curl -X POST https://api.amsydatasub.com/biometric/register/verify \
  -H "Content-Type: application/json" \
  -d @production-attestation.json

# ✅ RP ID hash matches
# Extract from production attestation and compare with SHA256("amsydatasub.com")
```
...
- ✅ Check backend is computing `sha256("amsydatasub.com")`
- ✅ Ensure RP ID in options matches backend (should be "amsydatasub.com")
